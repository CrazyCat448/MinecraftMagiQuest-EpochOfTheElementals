...
package net.mcreator.epochoftheelementals.world.biome;
...
import net.mcreator.epochoftheelementals.entity.RockSpiritEntity;
import net.mcreator.epochoftheelementals.block.ZelfiNylliumBlock;
import net.mcreator.epochoftheelementals.block.ZelfiFungusBlock;
...
public class ZelfiBiomeBiome extends EpochOfTheElementalsModElements.ModElement {
	@ObjectHolder("epoch_of_the_elementals:zelfi_biome")
	public static final CustomBiome biome = null;
	public ZelfiBiomeBiome(EpochOfTheElementalsModElements instance) {
		super(instance, 27);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(2f).precipitation(Biome.RainType.NONE)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(ZelfiNylliumBlock.block.getDefaultState(),
							Blocks.NETHERRACK.getDefaultState(), Blocks.NETHERRACK.getDefaultState())));
			setRegistryName("zelfi_biome");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(ZelfiFungusBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(ZelfiFungusBlock.block.getDefaultState()))).baseHeight(7)
			...
			addFeature(GenerationStage.Decoration.UNDERGROUND_ORES,
					Feature.DISK
							.withConfiguration(new SphereReplaceConfig(Blocks.GRAVEL.getDefaultState(), 6, 2,
									Lists.newArrayList(ZelfiNylliumBlock.block.getDefaultState(), Blocks.NETHERRACK.getDefaultState())))
							.withPlacement(Placement.COUNT_TOP_SOLID.configure(new FrequencyConfig(2))));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(RockSpiritEntity.entity, 1, 1, 1));
		}
	}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == ZelfiNylliumBlock.block.getDefaultState().getBlock() || ground == Blocks.NETHERRACK.getDefaultState().getBlock())
							&& (ground2 == ZelfiNylliumBlock.block.getDefaultState().getBlock()
									|| ground2 == Blocks.NETHERRACK.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Blocks.NETHERRACK.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == ZelfiFungusBlock.block.getDefaultState().getBlock()
												|| state.getBlock() == ZelfiFungusBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, ZelfiFungusBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, ZelfiFungusBlock.block.getDefaultState(), bbox);
							if (state.getBlock().isAir(state, world, genhPos) || state.getMaterial().blocksMovement() || state.isIn(BlockTags.LEAVES)
									|| state.getBlock() == ZelfiFungusBlock.block.getDefaultState().getBlock()
									|| state.getBlock() == ZelfiFungusBlock.block.getDefaultState().getBlock()) {
								if (genh > 0) {
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(-1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(-1, genh, 0), ZelfiFungusBlock.block.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(1, genh, 0), ZelfiFungusBlock.block.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, -1)))
										setTreeBlockState(changedBlocks, world, position.add(0, genh, -1), ZelfiFungusBlock.block.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, 1)))
										setTreeBlockState(changedBlocks, world, position.add(0, genh, 1), ZelfiFungusBlock.block.getDefaultState(),
												bbox);
						...
						for (int genh = position.getY() - 3 + height; genh <= position.getY() + height; genh++) {
							...
							for (int genx = position.getX() - k4; genx <= position.getX() + k4; genx++) {
								for (int genz = position.getZ() - k4; genz <= position.getZ() + k4; genz++) {
									...
									if (state.isIn(BlockTags.LEAVES) || state.getBlock() == ZelfiFungusBlock.block.getDefaultState().getBlock()) {
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												ZelfiFungusBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private void addVines(IWorld world, BlockPos pos, Set<BlockPos> changedBlocks, MutableBoundingBox bbox) {
			setTreeBlockState(changedBlocks, world, pos, ZelfiFungusBlock.block.getDefaultState(), bbox);
			int i = 5;
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == ZelfiFungusBlock.block.getDefaultState().getBlock()
					|| blockType == ZelfiFungusBlock.block.getDefaultState().getBlock()
					|| blockType == ZelfiNylliumBlock.block.getDefaultState().getBlock()
